                        ...
                        {categories.map((category) => (
                            <button
                                ...
                                className={`px-5 py-2.5 rounded-full text-sm font-medium transition-all duration-300 border ${activeCategory === category
                                    ...
                                    : "bg-card text-muted-foreground border-card-border hover:border-primary/30 hover:text-primary"
                                    }`}
                            >
                                {category}
                            </button>
                    ...
                    <div className="columns-1 sm:columns-2 lg:columns-3 gap-6 space-y-6">
                        {filteredImages.map((img, index) => (
                            <AnimatedSection key={`${img.src}-${activeCategory}`} delay={index * 0.05} className="break-inside-avoid">
                                <div className="shadow-sm hover:shadow-xl transition-all duration-300 rounded-2xl overflow-hidden group relative">
